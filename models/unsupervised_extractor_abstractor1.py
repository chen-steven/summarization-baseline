...
'''
Use gumbel softmax to decode unsupervised summary
...
class ExtractorAbstractorT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        if self.training:
            if self.config.teacher_forcing:
                ...
                gumbel_output = utils.gumbel_softmax_topk(sentence_logits.squeeze(-1), 5)
        else:
            ...
            gumbel_output = torch.topk(sentence_logits.squeeze(-1), 5, dim=-1)[1]
        ...
        ### Unsupervised Loop ###
        # gumbel softmax for greedy decoding of abstractive summary
        # re-encode this abstractive summary to compute similarity loss with original hidden states
        # also use original hidden states to decode original summary and compute reconstruction loss
        # maybe try reconstruction with original sentences
        ...
        if labels is not None:
            ...
            loss += 2 * -torch.mean(torch.sum(
                sentence_label_one_hot * torch.log_softmax(sentence_mask * (sentence_logits.squeeze(-1)), dim=-1),
                dim=-1))
    ...
    def prepare_inputs_for_generation(
            self, input_ids, decoder_sentence_indicator=None, decoder_sentence_labels=None, past=None,
